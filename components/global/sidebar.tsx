...
export default async function Sidebar() {
  ...
  return (
    <>
      <aside className='hidden lg:flex fixed  lg:h-screen w-16 flex-col bg-[#1e293b] text-[#94a3b8] sm:w-64'>
      ...
      <aside className='flex  w-full bottom-0 z-20 mt-40 fixed flex-col bg-[#1e293b] text-[#94a3b8] lg:hidden justify-center'>
        <nav className='flex-1 h-full flex flex-row gap-2 overflow-y-hidden px-2 py-4 sm:px-6 justify-center'>
          {shelves.map((shelf: Shelf) => (
            <Link
              key={shelf.id}
              href={`/books/${shelf.id}`}
